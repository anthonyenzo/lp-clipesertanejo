...
export default function Home() {
  ...
  return (
    <main>
      ...
      ].map((item, index) => <Reveal className="problem-card" delay={index * 70} key={item.n}><span className="card-number">{item.n}</span><div className="problem-icon">{item.icon === "play" ? <PlayIcon /> : item.icon === "wifi" ? <WeakWifiIcon /> : item.icon}</div><h3>{item.title}</h3><p>{item.text}</p></Reveal>)}</div></div></section>
      ...
      <section className="final-cta"><div className="final-glow" /><div className="shell final-inner"><Reveal><span className="eyebrow">Dê o play na sua experiência</span><h2>Mais de 500 clipes.<br /><span>Um clique de distância.</span></h2><p>Aproveite a oferta de hoje e receba seu acesso imediatamente após a confirmação.</p><a className="cta cta-large" href="#super-oferta">Garantir meu Pack agora <ArrowIcon /></a></Reveal></div></section>
      ...
      <a className="mobile-sticky-cta" href="#super-oferta"><span><small>Oferta a partir de</small><strong>R$ 16,90</strong></span><b>QUERO AGORA <ArrowIcon /></b></a>
    </main>
  );
}
